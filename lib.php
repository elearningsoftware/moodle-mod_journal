...
function journal_print_user_entry($course, $user, $entry, $teachers, $grades, $cmid)
{
    ...
    if ($entry) {
        ...
        // Feedback text area.
        $feedbackSection .= html_writer::label(fullname($user) . ' ' . get_string('feedback'), 'c' . $entry->id, true, ['class' => 'accesshide']);
        $feedbackSection .= html_writer::tag('p', html_writer::tag(
            'textarea',
            s($feedbacktext),
            ['id' => 'c' . $entry->id, 'name' => 'c' . $entry->id, 'rows' => 7, 'disabled' => $feedbackdisabledstr]
        ));
        if ($feedbackdisabledstr) {
            $feedbackSection .= html_writer::empty_tag('input', [
                'type' => 'hidden',
                'name' => 'c' . $entry->id,
                'value' => $feedbacktext
            ]);
    ...
    if ($entry) {
        $content .= html_writer::tag('p', html_writer::empty_tag('input', [
            'type' => 'button',
            'data-cmid' => $cmid,
            'data-entryid' => $entry->id,
            'data-userid' => $user->id,
            'value' => get_string('savefeedback', 'journal'),
            'class' => 'saveindividualfeedback btn btn-secondary m-t-1'
        ]), ['class' => 'feedbacksave', 'style' => 'margin-top: -16px;']);
    }
