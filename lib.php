...
function journal_scale_used($journalid, $scaleid) {
    ...
    $rec = $DB->get_record('journal', ['id' => $journalid, 'grade' => -$scaleid]);
...
function journal_grade_item_update($journal, $grades = null) {
    ...
    if (property_exists($journal, 'cmidnumber')) {
        $params = ['itemname' => $journal->name, 'idnumber' => $journal->cmidnumber];
    } else {
        $params = ['itemname' => $journal->name];
    }
...
function journal_count_entries($journal, $groupids = 0) {
    ...
    if (is_array($groupids) && !empty($groupids)) {     // How many in a particular group?
        ...
        $sql = 'SELECT DISTINCT u.id FROM {journal_entries} j
                ...
                WHERE j.journal = ?';
        $journals = $DB->get_records_sql($sql, [$journal->id]);
    }
...
function journal_print_user_entry($course, $user, $entry, $teachers, $grades, $cmid) {
    ...
    $content .= \html_writer::start_tag('table', ['class' => 'journaluserentry mb-1', 'id' => $entryid]);
    ...
    if ($entry) {
        $content .= \html_writer::start_tag('tr');
        $teacher = $teachers[$entry->teacher] ?? $DB->get_record('user', ['id' => empty($entry->teacher) ? $USER->id :
            $entry->teacher]);
        $content .= \html_writer::tag(
            'td',
        ...
        // Feedback text.
        echo \html_writer::label(fullname($user) . ' ' . get_string('feedback'), 'c' . $entry->id . '[text]', true);
        ...
        echo \html_writer::empty_tag('input', ['type' => 'hidden', 'name' => 'c' . $entry->id . '[itemid]',
            'value' => $draftitemid]);
    ...
    if ($entry) {
        ...
        echo 'value="' . get_string('savefeedback', 'journal') . '" class="saveindividualfeedback btn btn-secondary mt-1"/>';
        echo '</p>';
    }
